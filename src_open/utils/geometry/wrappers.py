...
from .utils import undistort_points, J_undistort_points, skew_symmetric#, so3exp_map
...
class Pose(TensorWrapper):
    ...
    @classmethod
    @autocast
    def from_identity(cls, n: int):
        R = torch.eye(3, 3).unsqueeze(0).expand(n, 3, 3)
        t = torch.zeros(size=(n, 3))
    ...
    def from_aa(cls, aa: torch.Tensor, t: torch.Tensor):
        ...
        return cls.from_Rt(axis_angle_to_matrix(aa), t)
    ...
    @property
    def aa(self) -> torch.Tensor:
        '''Underlying axis_angle vector with shape (..., 3).'''
        return quaternion_to_axis_angle(matrix_to_quaternion(self.R))
    ...
    @property
    def matrix_3x4(self) -> torch.Tensor:
    ...
    @autocast
    def rotate(self, p3d: torch.Tensor) -> torch:
        assert p3d.shape[-1] == 3
...
class Camera(TensorWrapper):
    ...
    # z: (b, n)
    # p3d: (b, n, 3)
    def unproject(self, p2d: torch.Tensor, z: torch.Tensor) -> Tuple[torch.Tensor]:
        valid = z > self.eps
        z = z.clamp(min=self.eps)
        p3d = (p2d - self.c.unsqueeze(-2)) / self.f.unsqueeze(-2) * z.unsqueeze(-1)
        p3d = torch.cat((p3d, z.unsqueeze(-1)), dim=2)
        return p3d, valid
    ...
    @autocast
    def view2image(self, p3d: torch.Tensor) -> Tuple[torch.Tensor]:
    ...
    @autocast
    def image2view(self, p2d: torch.Tensor, z: torch.Tensor) -> Tuple[torch.Tensor]:
        return self.unproject(p2d, z)
