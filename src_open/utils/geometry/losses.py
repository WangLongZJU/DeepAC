...
def error_add(vertices, gt_pose: Pose, pred_pose: Pose):
    gt_vertices = gt_pose.transform(vertices)
    pred_vertices = pred_pose.transform(vertices)
    # tmp = torch.norm(gt_vertices - pred_vertices, dim=-1).mean(dim=-1)
    # import ipdb;
    # ipdb.set_trace();
    return torch.sqrt(((gt_vertices - pred_vertices) ** 2).sum(dim=-1)).mean(dim=-1)

def error_add_s(vertices, gt_pose: Pose, pred_pose: Pose):
    gt_vertices = gt_pose.transform(vertices)
    pred_vertices = pred_pose.transform(vertices)
    dist = torch.cdist(gt_vertices, pred_vertices)
    dist_s, _ = dist.min(dim=-1)
    return dist_s.mean(dim=-1)