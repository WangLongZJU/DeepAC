...
class _Dataset(torch.utils.data.Dataset):
    def __init__(self, conf, split):
        ...
        for obj_name in self.obj_names:
            preprocess_path = os.path.join(self.root, 'Model3D', obj_name, 'pre_render', obj_name + '.pkl')
        ...
        self.diameters = {}
        for obj_name in self.obj_names:
            obj_path = os.path.join(self.root, 'Model3D', obj_name, obj_name + '.obj')
            ...
            # compute the object diameter from the maximum vertex distance
            vertex = vert * conf.geometry_unit_in_meter
            diameter_max = torch.max(torch.cdist(vertex, vertex))
            self.diameters[obj_name] = diameter_max
...
class _Dataset_test(_Dataset):
    ...
    def sample_new_items(self, seed):
        ...
        # get camera calibration from the website of OPT
        K = np.array([1060.197, 1060.273, 964.809, 560.952], dtype=np.float32)
        K = np.array([K[0], 0, K[2], 0, K[1], K[3], 0, 0, 1], dtype=np.float32)

        for obj_name in tqdm(self.obj_names):
            iter_slice = 0
            for slice in slices:
                data_dir = os.path.join(self.root, '3D')
                iter_slice += 1

                obj_short_name = obj_name[:2]
                pose_path = os.path.join(data_dir, 'poses', obj_short_name + '_' + slice + '.txt')
                pose_txt = np.loadtxt(pose_path)

                img_dir = os.path.join(data_dir, obj_short_name + '_' + slice, 'color')
                if not Path(img_dir).exists():
                    print("Warning: {} is not exist!".format(img_dir))
                    continue
                ...
                image_paths = {}
                for image_path in img_lists:
                    img_id = int(image_path.split('/')[-1].split('.')[0][-4:]) - 1 # imd_id from 0
                    image_paths[img_id] = image_path
                for img_id in range(len(img_lists)): 
                    ...
                    item = {'slice': slice, 'obj_id': obj_name, 'img_id': img_id, 'last_img_id': last_img_id,
                            ...
                            'last_body2view_t': last_body2view_t, 'output_name': output_name,
                            'end': True if img_id == len(img_lists)-1 else False, 'OPT': True,
                            'obj_start': True if (iter_slice == 1 and img_id == last_img_id) else False,
                            'obj_end': True if (iter_slice == len(slices) and img_id == len(img_lists)-1) else False,}
    ...
    def __getitem__(self, idx):
        ...
        obj_id = item['obj_id']
        body2view_R = item['body2view_R'].reshape(3, 3)
        body2view_R = body2view_R.T # different rotation format
        body2view_t = item['body2view_t']
        gt_body2view_pose = Pose.from_Rt(body2view_R, body2view_t)
        K = item['K']
        intrinsic_param = torch.tensor([ori_image.shape[1], ori_image.shape[0],
        ...
        last_body2view_R = item['last_body2view_R'].reshape(3, 3)
        last_body2view_R = last_body2view_R.T # different rotation format
        last_body2view_t = item['last_body2view_t']
        body2view_pose = Pose.from_Rt(last_body2view_R, last_body2view_t)
