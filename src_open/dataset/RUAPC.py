...
class _Dataset_RUAPC(torch.utils.data.Dataset):
    def __init__(self, conf, split):
        self.root = Path(conf.dataset_dir, 'ruapc')

        self.obj_names = conf.get('ruapc_' + split + '_obj_names')  # split: train/val
        self.pbr_slices = conf.get('ruapc_' + split + '_pbr_slices')  # split: train/val
        self.real_slices = conf.get('ruapc_' + split + '_real_slices')  # split: train/val
        ...
        for obj_name in self.obj_names:
            ...
            obj_id = str(int(obj_name.split('_')[-1]))
            self.vertices['ruapc_' + obj_id] = vert * conf.geometry_unit_in_meter
            ...
            self.diameters['ruapc_' + obj_id] = diameter[0]
    ...
    def sample_new_items(self, seed):
        ...
        # ---------------------------------------------------
        K_path = os.path.join(self.root, 'camera.json')
        with open(K_path, 'r', encoding='utf8') as fp:
            cam_K = json.load(fp)
        K = np.array([cam_K['fx'], 0.0, cam_K['cx'], 0.0, cam_K['fy'], cam_K['cy'], 0.0, 0.0, 1.0], dtype=np.float32)
        ...
        for pbr_slice in tqdm(pbr_slices):
            ...
            gt_dir = os.path.join(data_dir, 'gt', '000')
            img_dir = os.path.join(data_dir, 'rgb', '000')
            # mask_dir = os.path.join(data_dir, 'mask', '000')
            # mask_visib_dir = os.path.join(data_dir, 'mask_visib', '000')
            
            # outer loop
            sub_pbr_slices = os.listdir(gt_dir)
            sub_pbr_slices.sort()
            for sub_pbr_slice in tqdm(sub_pbr_slices):
                sub_gt_dir = os.path.join(gt_dir, sub_pbr_slice)
                sub_img_dir = os.path.join(img_dir, sub_pbr_slice)
                # sub_mask_dir = os.path.join(mask_dir, sub_pbr_slice)
                # sub_mask_visib_dir = os.path.join(mask_visib_dir, sub_pbr_slice)
                
                gt_lists = sorted(glob.glob(sub_gt_dir + '/*.yml', recursive=True))
                # inner loop
                for gt_path in gt_lists:
                    with open(gt_path, 'r') as fp:
                        gt_anno = yaml.safe_load(fp)
                    img_anno = list(gt_anno.values())[0]
                    # obj loop
                    for obj_anno in img_anno:
                        obj_id = obj_anno['obj_id']
                        px_count_visib = obj_anno['px_count_visib']
                        visib_fract = obj_anno['visib_fract']
                        if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                            (px_count_visib > self.conf.min_px_count_visib):

                            # image_path
                            image_id = os.path.basename(gt_path).split('.')[0]
                            image_name = image_id + '.png'
                            image_path = os.path.join(sub_img_dir, image_name)
                            ...
                            output_name = 'pbr_' + pbr_slice + '_' + sub_pbr_slice + '_ruapc_' + str(obj_id) + '_' + image_id

                            orientations_in_body = self.orientations_in_body['ruapc_' + str(obj_id)]
                            template_views = self.template_views['ruapc_' + str(obj_id)]
                            n_sample = self.num_sample_contour_points['ruapc_' + str(obj_id)]
                            diameter = self.diameters['ruapc_' + str(obj_id)]
                            vertex = self.vertices['ruapc_' + str(obj_id)]

                            item = {'slice': pbr_slice + '_' + sub_pbr_slice, 'obj_id': 'ruapc_' + str(obj_id), 'img_id': image_id,
                                'image_path': image_path, 'K': K, 'body2view_R': body2view_R, 'body2view_t': body2view_t,
                                'output_name': output_name, 'orientations_in_body': orientations_in_body, 'template_views': template_views,
                                'n_sample': n_sample, 'diameter': diameter, 'vertex': vertex, 'OPT': False}

                            if 'ruapc_' + str(obj_id) not in obj_items.keys():
                                obj_items['ruapc_' + str(obj_id)] = []
                            obj_items['ruapc_' + str(obj_id)].append(item)
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        body2view_R = np.array(obj_anno['cam_R_m2c'], dtype=np.float32)
                        body2view_t = np.array(obj_anno['cam_t_m2c'], dtype=np.float32) * self.geometry_unit_in_meter
                        output_name = 'real_' + real_slice + '_ruapc_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        ...
                        item = {'slice': real_slice, 'obj_id': 'ruapc_' + str(obj_id), 'img_id': image_id,
                                'image_path': image_path, 'K': K, 'body2view_R': body2view_R, 'body2view_t': body2view_t,
                                'output_name': output_name, 'orientations_in_body': orientations_in_body, 'template_views': template_views,
                                'n_sample': n_sample, 'diameter': diameter, 'vertex': vertex, 'OPT': False}
