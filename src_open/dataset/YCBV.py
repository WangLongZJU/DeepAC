...
class _Dataset_YCBV(torch.utils.data.Dataset):
    def __init__(self, conf, split):
        self.root = Path(conf.dataset_dir, 'ycbv')

        self.obj_names = conf.get('ycbv_' + split + '_obj_names')  # split: train/val
        self.pbr_slices = conf.get('ycbv_' + split + '_pbr_slices')  # split: train/val
        self.real_slices = conf.get('ycbv_' + split + '_real_slices')  # split: train/val
        self.plus_slices = conf.get('ycbv_' + split + '_plus_slices') # split: train/val
        ...
        for obj_name in self.obj_names:
            ...
            preprocess_path = os.path.join(self.root, 'models_eval', obj_name, 'pre_render', obj_name+'.pkl')
            obj_template_view_paths.append(preprocess_path)
            # add sub_dataset prefix 
            obj_ids.append('ycbv_' + obj_id)
        ...
        self.diameters = {}
        for obj_name in self.obj_names:
            obj_path = os.path.join(self.root, 'models_eval', obj_name+'.ply')
            ...
            obj_id = str(int(obj_name.split('_')[-1]))
            self.vertices['ycbv_' + obj_id] = vert * conf.geometry_unit_in_meter
            ...
            self.diameters['ycbv_' + obj_id] = diameter[0]
    ...
    def sample_new_items(self, seed):
        ...
        # add slices, including pbr_slices, real_slices and plus_slices
        # pbr_slices
        ...
        # plus_slices
        seq_dir = os.path.join(self.root, 'train_real')
        if self.plus_slices == 'none':
            plus_slices = []
        else:
            seq_list = os.listdir(seq_dir)
            seq_list.sort()
            if self.plus_slices == 'all':
                plus_slices = seq_list
            elif self.plus_slices == 'train_split':
                plus_slices = seq_list[:int(0.7*len(seq_list))]
            elif self.plus_slices == 'val_split':
                plus_slices = seq_list[int(0.7*len(seq_list)):]
            else:
                raise NotImplementedError            
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        output_name = 'pbr_' + pbr_slice + '_ycbv_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        
                        orientations_in_body = self.orientations_in_body['ycbv_' + str(obj_id)]
                        template_views = self.template_views['ycbv_' + str(obj_id)]
                        n_sample = self.num_sample_contour_points['ycbv_' + str(obj_id)]
                        diameter = self.diameters['ycbv_' + str(obj_id)]
                        vertex = self.vertices['ycbv_' + str(obj_id)]

                        item = {'slice': pbr_slice, 'obj_id': 'ycbv_' + str(obj_id), 'img_id': image_id,
                        ...
                        if 'ycbv_' + str(obj_id) not in obj_items.keys():
                            obj_items['ycbv_' + str(obj_id)] = []
                        obj_items['ycbv_' + str(obj_id)].append(item)
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        body2view_R = np.array(obj_anno['cam_R_m2c'], dtype=np.float32)
                        body2view_t = np.array(obj_anno['cam_t_m2c'], dtype=np.float32) * self.geometry_unit_in_meter
                        output_name = 'real_' + real_slice + '_ycbv_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        ...
                        item = {'slice': real_slice, 'obj_id': 'ycbv_' + str(obj_id), 'img_id': image_id,
                                'image_path': image_path, 'K': K, 'body2view_R': body2view_R, 'body2view_t': body2view_t,
                                'mask_path': mask_path, 'mask_visib_path': mask_visib_path, 'output_name': output_name,
                                'orientations_in_body': orientations_in_body, 'template_views': template_views, 
                                'n_sample': n_sample, 'diameter': diameter, 'vertex': vertex, 'OPT': False}
        ...
        # plus_slices
        for plus_slice in tqdm(plus_slices):
            data_dir = os.path.join(self.root, 'train_real', plus_slice)
            scene_anno_path = os.path.join(data_dir, 'scene_gt.json')
            with open(scene_anno_path, 'r', encoding='utf8') as fp:
                scene_anno = json.load(fp)
            scene_meta_anno_path = os.path.join(data_dir, 'scene_gt_info.json')
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        body2view_R = np.array(obj_anno['cam_R_m2c'], dtype=np.float32)
                        body2view_t = np.array(obj_anno['cam_t_m2c'], dtype=np.float32) * self.geometry_unit_in_meter
                        output_name = 'plus_' + plus_slice + '_ycbv_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        ...
                        item = {'slice': plus_slice, 'obj_id': 'ycbv_' + str(obj_id), 'img_id': image_id,
                                'image_path': image_path, 'K': K, 'body2view_R': body2view_R, 'body2view_t': body2view_t,
                                'mask_path': mask_path, 'mask_visib_path': mask_visib_path, 'output_name': output_name,
                                'orientations_in_body': orientations_in_body, 'template_views': template_views, 
                                'n_sample': n_sample, 'diameter': diameter, 'vertex': vertex, 'OPT': False}
