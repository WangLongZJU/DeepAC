...
class _Dataset_HOPE(torch.utils.data.Dataset):
    def __init__(self, conf, split):
        self.root = Path(conf.dataset_dir, 'hope')

        self.obj_names = conf.get('hope_' + split + '_obj_names')  # split: val
        self.pbr_slices = conf.get('hope_' + split + '_pbr_slices')  # split: val
        self.real_slices = conf.get('hope_' + split + '_real_slices')  # split: val
        ...
        for obj_name in self.obj_names:
            ...
            obj_id = str(int(obj_name.split('_')[-1]))
            self.vertices['hope_' + obj_id] = vert * conf.geometry_unit_in_meter
            ...
            self.diameters['hope_' + obj_id] = diameter[0]
    ...
    def sample_new_items(self, seed):
        ...
        # real_slices
        seq_dir = os.path.join(self.root, 'val')
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        output_name = 'pbr_' + pbr_slice + '_hope_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        
                        orientations_in_body = self.orientations_in_body['hope_' + str(obj_id)]
                        template_views = self.template_views['hope_' + str(obj_id)]
                        n_sample = self.num_sample_contour_points['hope_' + str(obj_id)]
                        diameter = self.diameters['hope_' + str(obj_id)]
                        vertex = self.vertices['hope_' + str(obj_id)]
                        
                        item = {'slice': pbr_slice, 'obj_id': 'hope_' + str(obj_id), 'img_id': image_id,
                        ...
                        if 'hope_' + str(obj_id) not in obj_items.keys():
                            obj_items['hope_' + str(obj_id)] = []
                        obj_items['hope_' + str(obj_id)].append(item)

        # real_slices
        for real_slice in tqdm(real_slices):
            data_dir = os.path.join(self.root, 'val', real_slice)
            scene_anno_path = os.path.join(data_dir, 'scene_gt.json')
            with open(scene_anno_path, 'r', encoding='utf8') as fp:
                scene_anno = json.load(fp)
            scene_meta_anno_path = os.path.join(data_dir, 'scene_gt_info.json')
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        body2view_R = np.array(obj_anno['cam_R_m2c'], dtype=np.float32)
                        body2view_t = np.array(obj_anno['cam_t_m2c'], dtype=np.float32) * self.geometry_unit_in_meter
                        output_name = 'real_' + real_slice + '_hope_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        ...
                        item = {'slice': real_slice, 'obj_id': 'hope_' + str(obj_id), 'img_id': image_id,
                                'image_path': image_path, 'K': K, 'body2view_R': body2view_R, 'body2view_t': body2view_t,
                                'mask_path': mask_path, 'mask_visib_path': mask_visib_path, 'output_name': output_name,
                                'orientations_in_body': orientations_in_body, 'template_views': template_views, 
                                'n_sample': n_sample, 'diameter': diameter, 'vertex': vertex, 'OPT': False}
