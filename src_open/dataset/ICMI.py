...
class _Dataset_ICMI(torch.utils.data.Dataset):
    def __init__(self, conf, split):
        self.root = Path(conf.dataset_dir, 'icmi')

        self.obj_names = conf.get('icmi_' + split + '_obj_names')  # split: val
        self.pbr_slices = conf.get('icmi_' + split + '_pbr_slices')  # split: val
        self.real_slices = conf.get('icmi_' + split + '_real_slices')  # split: val
        ...
        for obj_name in self.obj_names:
            ...
            obj_id = str(int(obj_name.split('_')[-1]))
            self.vertices['icmi_' + obj_id] = vert * conf.geometry_unit_in_meter
            ...
            self.diameters['icmi_' + obj_id] = diameter[0]
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        output_name = 'pbr_' + pbr_slice + '_icmi_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        
                        orientations_in_body = self.orientations_in_body['icmi_' + str(obj_id)]
                        template_views = self.template_views['icmi_' + str(obj_id)]
                        n_sample = self.num_sample_contour_points['icmi_' + str(obj_id)]
                        diameter = self.diameters['icmi_' + str(obj_id)]
                        vertex = self.vertices['icmi_' + str(obj_id)]
                        
                        item = {'slice': pbr_slice, 'obj_id': 'icmi_' + str(obj_id), 'img_id': image_id,
                        ...
                        if 'icmi_' + str(obj_id) not in obj_items.keys():
                            obj_items['icmi_' + str(obj_id)] = []
                        obj_items['icmi_' + str(obj_id)].append(item)
            ...
            for image_id in scene_anno:
                ...
                for i, (obj_anno, meta) in enumerate(zip(image_anno, meta_anno)):
                    ...
                    if (obj_id in self.obj_ids) and (visib_fract > self.conf.min_visib_fract) and \
                        ...
                        body2view_R = np.array(obj_anno['cam_R_m2c'], dtype=np.float32)
                        body2view_t = np.array(obj_anno['cam_t_m2c'], dtype=np.float32) * self.geometry_unit_in_meter
                        output_name = 'real_' + real_slice + '_icmi_' + str(obj_id) + '_' + os.path.basename(image_path).split('.')[0]
                        ...
                        item = {'slice': real_slice, 'obj_id': 'icmi_' + str(obj_id), 'img_id': image_id,
                                'image_path': image_path, 'K': K, 'body2view_R': body2view_R, 'body2view_t': body2view_t,
                                'mask_path': mask_path, 'mask_visib_path': mask_visib_path, 'output_name': output_name,
                                'orientations_in_body': orientations_in_body, 'template_views': template_views, 
                                'n_sample': n_sample, 'diameter': diameter, 'vertex': vertex, 'OPT': False}
